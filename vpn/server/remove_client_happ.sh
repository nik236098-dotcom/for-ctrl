#!/usr/bin/env bash
#
# Удаляет клиента Happ с сервера навсегда.
#
#   sudo bash vpn/server/remove_client_happ.sh tg123456
#
set -euo pipefail
...
jq --arg name "$NAME" 'map(select(.name != $name))' "$CLIENTS_DB" > "${CLIENTS_DB}.tmp"
